...
def generate_script(
    ...
    lines.append("\n")
    base = open(os.path.join(root_dir, "base_script_morecav.sh")).readlines()
...
configs = []
for i in range(14):
    configs.append("weather-%d-morecav.yaml" % i)
...
scen_params = 'leaderboard/scenarios/scenario_parameter_morecav.yaml'

for i, weather in enumerate(weathers_to_generate):
    if not os.path.exists(os.path.join(script_dir, "weather-%d-morecav" % weather)):
        os.mkdir(os.path.join(script_dir, "weather-%d-morecav" % weather))
    for route in routes:
        ...
        fw = open(
            os.path.join(script_dir, "weather-%d-morecav/%s.sh" % (weather, route.split("/")[2].split(".")[0])), "w"
        )
        for line in script:
            fw.write(line)
...
fw_all = open(os.path.join(root_dir, "generate_v2xverse_morecav_all.sh"), "w")
for _, weather in enumerate(weathers_to_generate):
    if not os.path.exists(os.path.join(batch_script_dir, "weather-%d-morecav" % weather)):
        os.mkdir(os.path.join(batch_script_dir, "weather-%d-morecav" % weather))
    for town in town_list:
        route_order = 0
        for i, port_len in enumerate(town_dict[town]['port_split']):
            fw = open(os.path.join(batch_script_dir, "weather-%d-morecav/town_%s_%s.sh" % (weather,town,i)), "w")

            for _ in range(port_len):
                route = 'routes_{}_{}'.format(town, route_order)
                fw.write("bash ./simulation/data_collection/scripts/weather-%d-morecav/%s.sh \n" % (weather, route))
                route_order += 1

            fw_all.write("bash ./simulation/data_collection/batch_run/weather-%d-morecav/town_%s_%s.sh & \n" % (weather,town,i))
